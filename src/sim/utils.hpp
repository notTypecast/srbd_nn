...
namespace srbd {
    ...
    */
    ...
    template <typename Derived> inline bool is_finite(const Eigen::MatrixBase<Derived>& x)
    {
    ...
    inline Matrix pinv(const Matrix& jac, double l = 0.01)
    {
        ...
        if (n >= m) {
            return jac.transpose()
                * (jac * jac.transpose() + l * l * Matrix::Identity(m, m)).inverse();
        }
