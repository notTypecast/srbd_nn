...
namespace srbd {
    ...
    */
    ...
    {
        // TO-DO: This is not the best way to handle this!! BUT should be okay
        // for now..
        operator=(other);
    }

    SingleRigidBodyDynamics& SingleRigidBodyDynamics::operator=(
        const SingleRigidBodyDynamics& other)
    {
        // COM state
    ...
    void SingleRigidBodyDynamics::set_feet_data(const std::vector<Vec3d>& feet_ref_positions,
        const std::vector<Vec3d>& feet_min_bounds, const std::vector<Vec3d>& feet_max_bounds)
    {
        assert((feet_ref_positions.size() == feet_min_bounds.size()
                   && feet_ref_positions.size() == feet_max_bounds.size())
            && "Not consistent number of feet!");
        _feet_ref_positions = feet_ref_positions;
        _feet_min_bounds = feet_min_bounds;
        _feet_max_bounds = feet_max_bounds;
    }

    void SingleRigidBodyDynamics::set_data(const Vec3d& base_position, const Vec3d& base_velocity,
        const RotMat& base_orientation, const Vec3d& base_angular_velocity,
        const std::vector<Vec3d>& feet_positions, const std::vector<size_t>& feet_phases)
    {
        // Base-related
        ...
        // Feet-related
        assert(feet_positions.size() == _feet_ref_positions.size()
            && feet_phases.size() == _feet_ref_positions.size() && "Wrong number of feet!");
    ...
    void SingleRigidBodyDynamics::set_base_orientation(const RotMat& rot)
    {
        _base_orientation = rot;
    }

    void SingleRigidBodyDynamics::set_base_angular_velocity(const Vec3d& ang_vel)
    {
        _base_angular_vel = ang_vel;
    }
    ...
    void SingleRigidBodyDynamics::set_dt(double dt) { _dt = dt; }
    ...
    std::tuple<Matrix, Matrix, Vector> SingleRigidBodyDynamics::linearize() const
    {
        ...
        Matrix L_c = skew(_base_angular_vel)
            * (_inertia - skew(_inertia * _base_angular_vel)); // optimal gain
        // State Space Model X_n+1 = A*X_n + B*U_n
        // Define A Matrix
        Matrix A = Matrix::Zero(n_dim, n_dim);
        // Block of size (p,q), starting at (i,j)	matri_block(i,j,p,q);
        // matri_block<p,q>(i,j);
        ...
        B.block(0, 0, 3, 3 * n_feet)
            = kron((((0.5 * _dt * _dt) / _mass) * Matrix::Ones(1, n_feet)), Matrix::Identity(3, 3));
        B.block(3, 0, 3, 3 * n_feet)
            = kron(((_dt / _mass) * Matrix::Ones(1, n_feet)), Matrix::Identity(3, 3));
        ...
        B.block(6, 0, 3, 3 * n_feet)
            = _dt * _inertia_inv * _base_orientation.transpose() * r_dtstack;
        ...
        d.segment(6, 3)
            = _dt * _inertia_inv * (L_c - skew(_base_angular_vel) * _inertia) * _base_angular_vel;
    ...
    {
        Vector state(12);
        state << _base_position, _base_vel, rotation_matrix_to_euler_zyx(_base_orientation),
            _base_angular_vel;
        return state;
    }
    ...
    std::pair<Matrix, Vec6d> SingleRigidBodyDynamics::inv_mass_matrix() const
    {
        ...
        for (size_t i = 0; i < n_feet; i++) {
            ...
            // Angular part
            M.block(3, i * 3, 3, 3) = _inertia_inv * _base_orientation.transpose()
                * skew(_feet_positions.at(i) - _base_position);
        }
    ...
    void SingleRigidBodyDynamics::integrate(
        const std::vector<Vec3d>& feet_forces, const Vec3d& external_force)
    {
        const size_t n_feet = _feet_ref_positions.size();
        Matrix M;
        Vec6d v;
    ...
    _terrain->friction_coeff(_feet_positions.at(k)[0],
    _feet_positions.at(k)[1]); Vec3d n, t1, t2; std::tie(n, t1, t2) =
    _terrain->normals(_feet_positions.at(k)[0], _feet_positions.at(k)[1]);
                C.block(k * 5, k * 3, 1, 3) << t1[0] + n[0] * friction_coeff,
    t1[1] + n[1] * friction_coeff, t1[2] + n[2] * friction_coeff; C.block(k * 5
    + 1, k * 3, 1, 3) << t1[0] - n[0] * friction_coeff, t1[1] - n[1] *
    friction_coeff, t1[2] - n[2] * friction_coeff; C.block(k * 5 + 2, k * 3, 1,
    3) << t2[0] + n[0] * friction_coeff, t2[1] + n[1] * friction_coeff, t2[2] +
    n[2] * friction_coeff; C.block(k * 5 + 3, k * 3, 1, 3) << t2[0] - n[0] *
    friction_coeff, t2[1] - n[1] * friction_coeff, t2[2] - n[2] *
    friction_coeff; C.block(k * 5 + 4, k * 3, 1, 3) << n[0], n[1], n[2];

                if (_phase_handler->is_swing(_feet_phases[k]))
                { // swing phase, z component should be zero
                    l(k * 5 + 4) = 0.;
                    u(k * 5 + 4) = 0.;
                }
                else
                {
                    l(k * 5 + 4) = 0.1 * _mass * _g /
    static_cast<double>(n_feet); // z-component should always be positive (and
    if in contact, it should produce at least a small force) u(k * 5 + 4) = 5. *
    _mass * _g;                                // max force
                }
    ...
    proxsuite::proxqp::QPSolverOutput::PROXQP_SOLVED) _last_qp_result_valid =
    false; else _last_qp_result_valid = true;
        }
        ...
        // Integrate
        Vector F = Eigen::Map<Vector>(const_cast<double *>(&feet_forces[0][0]),
    static_cast<int>(3 * n_feet));
    ...
    */
    ...
    {
        bool finite = is_finite(_base_position) && is_finite(_base_vel)
            && is_finite(_base_orientation) && is_finite(_base_angular_vel);
        if (!finite || !in_bounds())
            return false;
    ...
    SingleRigidBodyDynamics SingleRigidBodyDynamics::create_robot(RobotType type)
    {
        ...
        auto make_monoped = [&]() {
            ...
            auto robot = SingleRigidBodyDynamics(mass,
                inertia_tensor(mass * (by * by + bz * bz) / 12., mass * (bx * bx + bz * bz) / 12.,
                    mass * (bx * bx + by * by) / 12., 0., 0., 0.),
                feet_ref_positions, feet_min_bounds, feet_max_bounds, T, T_swing);
        ...
        auto make_quadruped = [&]() {
            ...
            robot.set_base_position(Vec3d(0., 0., initial_height));
            robot.set_feet_positions({Vec3d(bx / 2., by / 2., 0.), Vec3d(-bx / 2., by / 2., 0.),
                Vec3d(bx / 2., -by / 2., 0.), Vec3d(-bx / 2., -by / 2., 0.)});
        ...
        auto make_hexapod = [&]() {
            ...
            robot.set_base_position(Vec3d(0., 0., initial_height));
            robot.set_feet_positions({Vec3d(bx / 2., by / 2., 0.), Vec3d(-bx / 2., by / 2., 0.),
                Vec3d(bx / 2., -by / 2., 0.), Vec3d(-bx / 2., -by / 2., 0.), Vec3d(0., by / 2., 0.),
                Vec3d(0., -by / 2., 0.)});
        ...
        auto make_anymal = [&]() {
            ...
            // This from towr
            // RotMat inertia = inertia_tensor(0.946438, 1.94478, 2.01835,
            // 0.000938112, -0.00595386, -0.00146328); const double anymal_mass
            // = 29.5; This is from our URDF
            RotMat inertia = inertia_tensor(
            ...
            // NOTE: Right back
            feet_positions.push_back(Vec3d(-x_nominal_b, -y_nominal_b, 0.));
            feet_min_bounds.push_back(
                Vec3d(-x_nominal_b - dx, -y_nominal_b - dy, z_nominal_b - dz));
            feet_max_bounds.push_back(
                Vec3d(-x_nominal_b + dx, -y_nominal_b + dy, z_nominal_b + dz));
            feet_ref_positions.push_back(Vec3d(-x_nominal_b, -y_nominal_b, z_nominal_b));

            auto robot = SingleRigidBodyDynamics(anymal_mass, inertia, feet_ref_positions,
                feet_min_bounds, feet_max_bounds, T, T_swing);
    ...
    void SingleRigidBodyDynamics::_integrate(const Vec6d& acc)
    {
        ...
        for (size_t i = 0; i < n_feet; i++) {
            ...
            if (_phase_handler->is_phase_change(_feet_phases[i])) {
                _feet_positions[i].head(2) = _feet_ref_positions[i].head(2)
                    + k_foot * (_base_orientation.transpose() * _base_vel).head(2);
                _feet_positions[i][2] = 0.;

                // _feet_positions[i][0] = std::max(_feet_min_bounds[i][0],
                // std::min(_feet_max_bounds[i][0], _feet_positions[i][0]));
                // _feet_positions[i][1] = std::max(_feet_min_bounds[i][1],
                // std::min(_feet_max_bounds[i][1], _feet_positions[i][1]));
                // _feet_positions[i][2] = std::max(_feet_min_bounds[i][2],
                // std::min(_feet_max_bounds[i][2], _feet_positions[i][2]));

                _feet_positions[i] = _base_position + _base_orientation * _feet_positions[i];
                _feet_positions[i][2] = _terrain->height(_feet_positions[i][0],
                    _feet_positions[i][1]); // get height from terrain
            }
        }
    }
} // namespace srbd