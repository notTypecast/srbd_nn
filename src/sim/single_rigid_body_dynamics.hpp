#ifndef SRBD_SINGLE_RIGID_BODY_DYNAMICS_HPP
#define SRBD_SINGLE_RIGID_BODY_DYNAMICS_HPP
...
namespace srbd {
    enum RobotType { Monoped = 0, Biped, Quadruped, Hexapod, ANYmal };
    ...
    struct FlatTerrain : public BaseTerrain {
        ...
        // Terrain function
        double height(double /* x */, double /* y */) const override { return z; }
    ...
    struct CubicHeightMapTerrain : public BaseTerrain {
        ...
        CubicHeightMapTerrain(
            double xa, double xb, double xc, double xd, double ya, double yb, double yc, double yd)
            : a1(xa), b1(xb), c1(xc), d1(xd), a2(ya), b2(yb), c2(yc), d2(yd)
        {
        }
    ...
    public:
        ...
        SingleRigidBodyDynamics(double mass, const RotMat& inertia,
            const std::vector<Vec3d>& feet_ref_positions, const std::vector<Vec3d>& feet_min_bounds,
            const std::vector<Vec3d>& feet_max_bounds, Args... args)
        {
            // Body-related
        ...
        void set_feet_data(const std::vector<Vec3d>& feet_ref_positions,
            const std::vector<Vec3d>& feet_min_bounds, const std::vector<Vec3d>& feet_max_bounds);

        // Changing quantities
        void set_data(const Vec3d& base_position, const Vec3d& base_velocity,
        ...
        // PhaseHandler
        template <typename T, typename... Args> void set_phase_handler(Args... args)
        {
            _phase_handler.reset(new T{args...});
        }

        // Terrain
        template <typename T, typename... Args> void set_terrain(Args... args)
        {
        ...
        /// ForwardSim-related
        // linearization function #2 [M*f + v] gives produced acceleration
        // (useful for forward sim)
